...
class PhoneTracking:
    ...
    def __init__(self):
        ...
        self.camera_depth_sub = rospy.Subscriber("/front/depth/image_rect_raw", Image, self.camera_depth_callback)
        self.camera_img_sub = rospy.Subscriber("/front/color/image_raw",Image, self.image_tracker )
        self.camera_info_sub = rospy.Subscriber("/front/color/camera_info", CameraInfo, self.camera_info_callback)
        self.is_phone_present_pub = rospy.Publisher("/front/is_phone_present", Bool, queue_size = 30)
        self.tracked_image_pub = rospy.Publisher("/front/tracked_image", Image, queue_size = 30)
        self.phone_pose_camera_pub = rospy.Publisher("/front/phone_pose_camera_raw", PoseStamped, queue_size = 70)
        self.projection_mat = None
        self.inv_projection_mat = None
        if os.path.exists("/home/biorobotics/wombat_arm_ws/src/wombat_tracking/src/projection_mat.npy"):
            self.projection_mat = np.load("/home/biorobotics/wombat_arm_ws/src/wombat_tracking/src/projection_mat.npy")
            print("Projection Matrix = ",self.projection_mat)
        if os.path.exists("/home/biorobotics/wombat_arm_ws/src/wombat_tracking/src/inv_projection_mat.npy"):
            self.inv_projection_mat = np.load("/home/biorobotics/wombat_arm_ws/src/wombat_tracking/src/inv_projection_mat.npy")
            print("Inv Projection Matrix = ",self.inv_projection_mat)
        self._bbox_pub = rospy.Publisher("/front/bbox", BoundingBox3D, queue_size=30)
            ...
            if detector.phone_present==True:
                ...
                if len(boxes)>0:
                    for i, box in enumerate(boxes):
                        min_area_rect = min_area_rects[i]
                        ...
                        # if item_class is phone
                        # if width >height:
                        #     orientation = orientation - 90

                        # orientation =90 - orientation
                            # print(f"transoformed_orientation {orientation}")
                        ...
                        bbox_msg.center = phone_pose.pose
                        bbox_msg.size.x = width
                        bbox_msg.size.y = height
